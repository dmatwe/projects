...
<br/>
[1. Балансировщик нагрузки](#м1)
<br/>
[2. Распределение данных](#м2)
<br/>
[3. Секционирование](#м3)
<br/>
[4. Избыточность](#м4)
<br/>
[5. Репликация](#м5)
<br/>
[6. Консистентное (согласованное) хеширование](#м6)
<br/>
[7. Масштабирование сервиса соц сети](#м7)
<br/>
[8. Заключение](#м8)

**Повышение отзывчивости**
<br/>
[1. Кэширование данных](#о2)
<br/>
[2. Content Delivery Network](#о3)
<br/>
[3. Индексирование баз данных](#о4)
<br/>
[4. Создание ID записей](#о5)
<br/>
[5. Соединения и протоколы](#о6)
<br/>
[6. Повышение отзывчивости сервиса соц сети](#о7)
<br/>
[7. Заключение](#о8)

**Подсистемы для поиска**
<br/>
[1. Реализация автодополнения](#п1)
<br/>
[2. Префиксное дерево](#п2)
<br/>
[3. Поиск подстроки в строке](#п3)
<br/>
[4. Префикс-функция](#п4)
<br/>
[5. Алгоритм Ахо-Корасик](#п5)
<br/>
[6. Автодополнение строки. Реализация](#п6)
<br/>
[7. Текстовый поиск. Реализация](#п7)
<br/>
[8. Поиск вхождений по словарю. Реализация](#п8)
<br/>
[9. Неточный поиск (wildcard matching)](#п9)
<br/>
[10. Готовый движок для поиска](#п10)
<br/>
[11. Поиск по геолокации](#п11)
<br/>
[12. GeoHash](#п12)
<br/>
[13. QuadTree](#п13)
<br/>
[14. GeoHash VS QuadTree](#п14)
<br/>
[15. Архитектура сервиса автодополнения](#п15)
<br/>
[16. Архитектура сервиса текстового поиска](#п16)
<br/>
[17. Архитектура сервиса поиска по геолокации](#п17)
<br/>
[18. Заключение](#п18)
<br/>

**Подсистемы ограничения**
<br/>
[1. Виды ограничений нагрузки на систему](#ог1)
<br/>
[2. Размещение модуля ограничения](#ог2)
<br/>
[3. Требования к модулю ограничения нагрузки](#ог3)
<br/>
[4. Алгоритм ведро с токенами](#ог4)
<br/>
[5. Алгоритм протекающего ведра](#ог5)
<br/>
[6. Алгоритм с фиксированным окном](#ог6)
<br/>
[7. Алгоритм с обновленным логом](#ог7)
<br/>
[8. Алгоритм со скользящим окном](#ог8)
<br/>
[9. Модули защиты системы](#ог9)
<br/>
[10. Работа прокси](#ог10)
<br/>
[11. Файрвол](#ог11)
<br/>
[12. Внешние сервисы](#ог12)
<br/>
[13. Подсистемы мониторинга](#ог13)
<br/>
[14. Внешние подсистемы. Взаимодействие ML](#ог14)
<br/>
[15. Внешние проблемы.  Сервис логирования](#ог15)
<br/>
[16. Заключение](#ог16)
...
**Масштабирование сервиса соц сети**
<a name="м7"></a>
...
2. Большая часть нагрузки приходится на малую часть запросов (принцип Парето 80 — 20)

Вместо обращения к сервису можно получать быстрый ответ из кэша


![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cash.png)

Кэш может располагаться практически на любом из уровней:

1. На запрос к фронтенду отдаем уже собранную страницу;
2. Для запроса на вычисления предоставляем сразу готовый ответ;
3. Для данных используем RAM вместо SSD, SSD вместо HDD и HDD вместо сети.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cash1.png)


***Способы инвалидации кэша:***


Рано или поздно данные в исходной БД изменятся и кэшированные данные придут в негодность. В таком случае необходимо произвести инвалидацию кэша.

***Сквозная запись: данные сразу записываются и в источник, и в кэш.***
1. Плюсы: гарантированная консистентность, минимизация потерь.
2. Минусы: запись становится даже медленнее из-за двух операций.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cash2.png)


***Запись в обход: данные записываются напрямую в источник.***

1. Плюсы: не нагружаем кэш записью невостребованных данных.
2. Минусы: для недавних данных считывать нечего, идём в источник.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cash3.png)

***Реверсивная запись: данные сначала записываются в кэш.***

1. Плюсы: низкие задержки и высокая пропускная способность на запись.
2. Минусы: можно потерять недавние данные, не продублированные в источник.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cash4.png)
...
5. Least Frequently Used (LFU): ведем счёт обращений, удаляем самые непопулярные;
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/cdn.png)
...
В случае неизбежного обращения к исходным данным в БД рано или поздно и это хочется ускорить. На помощь приходит индексирование БД, которое ускоряет поиск нужных записей в таблице.  
...
Особенно полезно в случае большого количества маленьких записей или распределённой БД.

***Плюсы:***
1. Увеличение скорости поиска записей по ключу в индексе (O(N) -> O(logN));
2. Ускоренный выбор нужного шарда в случае распределённой БД.

***Минусы:***
1. При обновлении данных нужно обновить не только их, но и записи в индексе;
2. Как итог, уменьшается скорость записи, обновления и удаления записей в таблице.

***Когда применяем:***

1. Поиск по многочисленным или распределённым записям в БД становится бутылочным горлышком.

***Когда не применяем:***

1. Если операций на запись значительно больше операций на чтение, то индексирование только во вред.
2. Лишние индексы не строим, а более ненужные не забываем удалять.
...
В случае разделения таблиц по нескольким машинам простого автоинкремента на 1 уже не хватит. 


***Самостоятельная генерация инстансом***

Создание ключей каждым основным инстансом БД самостоятельно. Мы всё также инкрементируем идентификаторы, но с заданным шагом больше 1. 

***Плюсы***
1. Плюс такого подхода в том, что он масштабируется под нужное количество серверов.

***Минусы***
1. Идентификаторы могут «разбежаться» со временем и стать не сравнимыми между серверами;
2. Масштабирование подхода статично и сложно подстроиться под изменение количества машин.


![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/idgen.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/uuid.png)


***Плюсы:***
1. Идентификаторы создаются независимо, основными репликами базы данных;
2. Легко масштабировать, т.к. нет необходимости в координации между ними.

***Минусы:***
1. 128 бит по стандарту больше 64-х бит;
2. Идентификаторы не линейны по времени;
3. Идентификаторы могут быть не целочисленными.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/gens.png)

***Плюсы:***
1. Получаем числовые упорядоченные линейно упорядоченные по времени ID;
2. Подход легко масштабируется для систем не сильно большого объема.

***Минусы:***
1. Генератор является единой точкой отказа в системе, без него ничего не записать в систему;
2. Для добавления надёжности нужно будет отдельно думать про избыточность и синхронизацию.


***Twitter генератор ID***

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/twitgen.png)

1. (1) Зарезервированный бит;
2. (41) Временную метку с момента начала работы Твиттера;
3. (5) Идентификатор датацентра;
4. (5) Идентификатор машины в датацентре;
5. (12) Последовательный номер в рамках миллисекунды по модулю ДЦ/машины.

***Плюсы:***
1. Помещаемся в 64 бита;
2. Все ID генерируются независимо;
3. Все ID упорядочены с точностью до секунд (зависит от NTP сервера);
4. Каждая машина в теории может генерировать несколько миллионов ID в секунду;
5. Упрощается маршрутизация при доступе к данным, тем самым в целом повышается отзывчивость.
...
4. Сервер отправляет ответ по открытому соединению.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/con.png)
...
1. Клиент открывает соединение и запрашивает страницу по HTTP;
2. Уже со страницы затем поступают запросы с интервалом;
3. Сервер подготавливает ответ на каждый такой запрос;
4. Клиент повторяет данный сценарий взаимодействия с другой страницей.

***Минусы подхода AJAX Polling для обновления данных на клиенте:***

1. Необходимо постоянно опрашивать сервер вне зависимости от наличия обновлений;
2. Большая часть ответов приходит пустая, забивая при этом HTTP канал.
...
1. Клиент открывает соединение и запрашивает страницу по HTTP;
2. Уже со страницы затем в свою очередь поступают запросы к серверу;
3. При наличии обновлений сервер посылает ответ на запрос клиенту;
4. Клиент затем почти сразу посылает новый такой же запрос;
5. У каждого установленного соединения есть таймаут, чтобы они не висели бесконечно.

***Веб-сокеты***

Еще одним способом взаимодействия между клиентом и сервером является использования веб-сокетов:

1. Соединение устанавливается клиентом через специальную процедуру (handshake);
2. Веб-сокеты предоставляют долгоиграющее соединение между клиентом и сервером;
3. И клиент, и сервер могут обмениваться данными в произвольном порядке;
4. Коммуникация обеспечивается с минимальными издержками.
5. Фактически веб-сокеты позволяют установить двусторонний диалог между клиентом и сервером.


![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/con2.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/con3.png)

Взаимодействие между клиентом и серверов через SSEs устроено следующим образом:

1. Клиент открывает соединение и запрашивает страницу по HTTP;
2. Открытая страница открывает соединение с сервером;
3. Сервер посылает данные клиенту в случае наличия обновлений;
4. Этот способ подходит для систем, где сервер присылает клиенту обновления в реальном времени.


**Повышение отзывчивости сервиса соц сети**
<a name="о7"></a>

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/fbs.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/fbs1.png)

1. Установим между пользователем и сервером кэш, чтобы каждый раз заново не запрашивать страницы. 
2. Когда запрашиваем профили, можем обращаться к кэшу, где, например, хранятся недавние профили. Если есть необходимость обратиться к исходной базе, можем ее проиндексировать.
3. У upload сервиса индекс стоит под знаком вопроса, так как, конечно, он может понять, в какой шард стоит загрузить файл. При этом он замедляет обращение к таблице с метаданными, так как там обычно чтение происходит чаще, чем запись.
4. У сервиса рекомендаций кэш также со знаком вопроса. Redis и так довольно быстрое хранилище, поэтому кэш не сильно поможет.

***Выводы***
<a name="о8"></a>

1. Для повышения отзывчивости помимо масштабирования можно кэшировать запросы и данные;
2. Упростить доступ к данным в крупных гео-распределенных системах может применение CDN;
3. Неизбежные обращения к исходным данным в БД можно ускорить за счёт построения индекса;
4. Балансировку и маршрутизацию запросов можно облегчить при правильном дизайне ID в БД;
5. В случае постоянного обмена между клиентом и сервером есть альтернативы частым запросам;
6. Чтобы оставаться на нужном уровне отзывчивости, можно применить ограничитель запросов.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/queryc.png)
...
1. на каждую введённую букву мы посылаем отдельный запрос;
2. каждый раз сканируем всю таблицу, а это O(N) или O(logN), если построен индекс.


***Префиксное дерево***
<a name="п2"></a>

Префиксное дерево (бор или trie) — структура данных для компактного хранения строк, 
<br/>
устроенная в виде дерева, где на рёбрах между вершинами записаны символы, 
<br/>
а некоторые вершины помечены терминальными. 
<br/>
Говорят, что префиксное дерево принимает строку s, если существует такая терминальная вершина $v$, 
<br/>
что, если выписать подряд все буквы на путях от корня до v, 
<br/>
то получится строка s.


***Нагруженное дерево*** отличается от обычных n-арных деревьев тем, что в его узлах не хранятся ключи. 
<br/>
Вместо них в узлах хранятся односимвольные метки, 
<br/>
а ключём, который соответствует некоему узлу, является путь от корня дерева до этого узла, 
<br/>
а точнее строка, составленная из меток узлов, повстречавшихся на этом пути. 
<br/>
В таком случае корень дерева, очевидно, соответствует пустому ключу.

Пример нагруженного дерева с ключами c, cap, car, cdr, go, if, is, it

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/tree.png)
...
Временная сложность этого алгоритма равна О (|Key|).

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/treesearch.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/treeins.png)

***Удаление***

Временная оценка алгоритма удаления —  О (|Key|).
...
1. Сжатие. Сжатое нагруженное дерево получается из обычного удалением промежуточных узлов, которые ведут к единственному не промежуточному узлу. Например, цепочка промежуточных узлов с метками a, b, c заменяется на один узел с меткой abс.
2. Patricia. Patricia нагруженное дерево получается из сжатого (или обычного) удалением промежуточных узлов, которые имеют одного ребенка.
...
1. Плюс: простота реализации.

2. Минус: при частичных совпадениях сложность возрастает до O( | haystack | x | needle | ).


***Префикс-функция***
<a name="п4"></a>

Префикс-функцией от строки S называется массив p, где p(i) равно длине самого большого префикса строки s(0)s(1)s(2)…s(i), который также является и суффиксом i-того префика (не считая весь i-й префикс).

Для строки 'abcdabcabcdabcdab' вычисление будет таким:  

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/pre.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/prepy.png)
...
Недетерминированный автомат для словаря {a, ab, bc, bca, c, caa}. 

Серые вершины промежуточные, 

белые конечные. 

Синие стрелки — суффиксные ссылки, 

зелёные — конечные.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/fish.png)


***Автодополнение строки. Реализация***
<a name="п6"></a>

Для оптимального хранения статистики со строками и поиска рассмотрим структуру данных ***«префиксное дерево»***.

Каждому слову из истории вызовов соответствует путь от корня к листьевой вершине дерева.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/treep.png)


Статистику по запросам можно хранить в самих вершинах, передвигая счётчик каждого слова, по соответствующему запросу.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/treep2.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/treep3.png)


***Текстовый поиск. Реализация***
<a name="п7"></a>
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/texts.png)
...
Символ ? соответствует одной любой букве, 

тогда как * соответствует любому количеству букв.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/wild.png)


Для оптимального решения задачи воспользуемся динамическим программированием. Построим матрицу

D размера S x P, где D[m][n] – матч между S[1..m] и P[1..n].
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/wild2.png)
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/aps.png)


Например, Apache Lucene представляет из себя движок на Java с биндингами для Python и позволяет:

1. Создавать индекс для поиска в документах с текстовыми и строковыми полями — выполнять запросы с определённым для движка синтаксисом;
2. Искать вхождения слов и запросов в полях (“title:курс”);
3. Добавлять простые логические операции в запрос;
4. Находить вхождения на расстоянии друг от друга (“karpovdesign”~4);
5. Задавать диапазоны для значений (start_date:[20220601 TO 20220704]).
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/geotab.png)


Если пользователь находится в точке 41.7167, 44.7853, то заведения в радиусе R (почти) надо найти запросом.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/qsql.png)

***Проблема такого подхода*** — в необходимости сканировать всю таблицу с заведениями целиком.

Можно ускорить поиск за счёт индексирования, но только в одном измерении. 

Полученные «полосы» всё равно придётся дорого JOIN’ить.

Для того, чтобы уменьшить зону поиска, карту можно равномерно разбить на квадраты.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/geoq.png)
...
Для поиска точки на карте или набора каких-то точек, оптимально разбить карту на отдельные квадраты, 
<br/>
каждому из которых задавать адрес. 
<br/>
При этом большие квадраты разбиваются на более мелкие 
<br/>
с постепенным увеличением адреса последних.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/geohash.png)
...
1. Если префикс у двух точек совпадает до какой-то степени, это гарантирует их близость;
2. Обратное не верно — достаточно близкие точки могут не иметь общего префикса вообще;
3. Найти ближайшие места можно простым SELECT * FROM places WHERE geohash LIKE ‘prefix%' ;
4. Также два заведения совсем рядом могут иметь разный хеш, поэтому стоит включать соседние клетки;
5. При необходимости расширить зону поиска достаточно немного уменьшить префикс;
6. Можно создавать локальный геохэш для областей присутствия.
...
1. разбиение пространства на адаптирующиеся ячейки (adaptable cells);
2. максимально возможный объём каждой ячейки;
3. соответствие направления дерева пространственному разбиению.
4. Предлагаемый подход также разбивает карту на квадраты, но делает это более оптимально — динамически увеличивает глубину разбиения в зависимости от количества точек

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/4geo.png)


***Особенности использования QuadTree для поиска ближайших локаций:***

1. Построение дерева линейно по количеству точек интереса;
2. Соответствие между искомой позицией и листом в дереве находится за считанные шаги;
3. Для поиска ближайших точек интереса либо останавливаемся в листе, либо поднимаемся выше;
4. Два заведения совсем рядом могут попасть в разные ячейки, поэтому можно взять соседние;
5. При необходимости расширить зону поиска достаточно подняться на один уровень выше;
6. Расширение зоны поиска происходит в неравной степени по сторонам света и по диагонали;
7. Все решение легко умещается в оперативную память и поиск проходит стремительно.

***GeoHash VS QuadTree***
<a name="п14"></a>

***Особенности использования геохеширования:*** 
1. легко реализовать, не надо ничего «строить»;
2. легко найти заведения (точки интереса) в определённом радиусе;
3. нельзя адаптировать гранулярность сетки, только перестраивать заново;
4. для добавления/удаления записи в индексе достаточно добавить/удалить одну строчку в таблице.
 

***Особенности использования квадродерева:***
1. чуть сложнее реализовать, т.к. нужно строить дерево поиска;
2. легко найти top-k ближайших точек интереса;
3. можно адаптировать глубину тех или иных участков дерева со временем;
4. добавление/удаление записей включает спуск по дереву и балансировку при необходимости.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/autoadd.png)

1. Search Serive обрабатывает входящие запросы
2. Logging Service логирует данные и хаписывает их в БД по шардам
3. Indexing Service забирает данные из БД за определенный временной интревал и строит префиксное дерево
4. Префиксное дерево хранится в отдельной БД, а топовые результаты в кеше
5. Autocomplete Service обращается к кешу/БД префиксного дерева для автодополнения запроса

***Подсистема для индексации и предоставления API для текстового поиска:***
<a name="п16"></a>

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/textparcer.png)

1. Тексты хранятся в БД на разных шардах/репликах 
2. Indexers индексируют все тексты и складывают в распределенные системы
3. Распределенные системы представляют собой шарды БД для поиска и сервисы, которые отвечают за каждую часть данных
4. Search UI - в зависимости от вводимого запроса пользователя балансировщик направляет его на нужный шард с данными

***Подсистема для индексации и предоставления API для поиска по геолокации:***
<a name="п17"></a>

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/spot.png)

1. Editor UI - редакторы заносят места для бронирования ресторанов / отелей
2. POI Service - сервис, который принимает запросы от редакторов
3. POI DB - Места хранятся в БД на разных шардах
4. Indexing Service - обращается к POI DB и строит GeoHash / QuadTree 
5. GeoHash / QuadTree хранится в Geo Index
6. Search UI - пользователь обращается к Geo Search Service
7. PIO Cache/DB выдает описание фото мест, Geo Index - информацию о ближайших точках

***Итоги***
<a name="п18"></a>

1. Популярными сценариями поиска являются автодополнение, поиск по тексту и геолокации;
2. Для автодополнения есть готовый алгоритм, производящий выдачу топа за константное время;
3. Для поиска по тексту есть как готовые алгоритмы для простых случаев, так и поисковые движки;
4. Простая фильтрация по координатам для поиска точек в окрестности неэффективна;
5. Для поиска по геоданным существуют различные решения на основе хешей или деревьев.
...
1. Количество запросов от одного пользователя;
2. Количество запросов от одного устройства;
3. Количество запросов от одного IP-адреса.


***Некоторые механизмы ограничений служат и целям безопасности:***

Ограничение количества неудачных попыток предотвращает атаку грубой силы (brute-force), 
<br/>
когда злоумышленники хотят получить доступ к данным путём перебора паролей;
...
1. На клиентской части приложения. Вариант не надёжный: запросы к серверу можно отсылать не только из приложения, а еще из других специальных приложений, например, с Postman. Такие запросы пройдут мимо ограничителя;

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/rate0.jpg)

2. На серверной части — вариант хороший, но дополнительно нагружает сервер основного приложения;

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/rate.jpg)


3. Самым оптимальным вариантом остается вынести ограничитель в отдельный модуль между клиентом и сервером. Такой вариант снимает нагрузку с основного сервиса и одновременно служит буфером для любых запросов.

![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/rate1.jpg)
...
1. Возможность корректно ограничить запросы к конкретному api в количестве N за время T;
2. Ограничитель должен быть доступен распределённо и учитывать нагрузку с разных серверов или процессов;
3. Ограничитель должен быть доступен всегда;
4. Применение ограничителя не должно сильно сказываться на быстродействии.
...
1. Для контейнера с токенами используется стек;
2. Изначально стек заполнен;
3. Устанавливается периодичность добавления в стек токена;
4. При каждом запросе от пользователя из стека удаляется один токен;
5. При исчерпании токенов запросы от пользователя не принимаются до момента пополнения стека новым токеном.
...
Если большое количество пользователей в конкретный момент времени начнут тратить свои токены, приложение получит максимальную нагрузку и может сломаться.
...
1. Логируем каждый успешный запрос от пользователя;
...
1. Гибридный подход между фиксированным окном и обновлённым логом;
...
1. Адреса реальных серверов скрывают за обратными прокси;
2. Файрвол для ограничения доступа извне.
...
Файрволы ограничивают возможность обработки запросов.
<br/>
В модуле устанавливают конкретные IP-адреса, порты или конкретных пользователей, с которых можно обрабатывать запрос.
<br/>
...
1. Пакетные фильтры — самые простые, сверяют IP, порт, протокол и другие базовые свойства запроса;
2. Сеансового уровня — соединяются от имени внутренних сервисов, пропускают пакеты только для них;
3. Инспекторы состояний — контролируют пакеты, сессии и приложения на основе внутренних таблиц разрешения.
4. Есть возможность использовать услуги внешних сервисов, например Cloudflare.
...
1. Распределение времени ответа тех или иных подсервисов;
2. Распределение положительных ответов и различных ошибок.
...
3. Grafana — система с удобным интерфейсом для мониторинга показателей. Сбор самих метрик можно осуществлять силами Graphite или Prometheus.
...
1. проводить различные А/Б тесты по привлекательности той или иной страницы;
2. заниматься офлайн сегментацией пользователя по интересам;
3. планировать перейти на Hadoop для обучения сложных моделей и их внедрения.
...
1. Ранее уже обсуждали критичность ACID-требований для чувствительных транзакций;
2. Сам процесс оплаты всегда можно возложить на внешние сервисы оплаты типа Stripe;
3. Логировать все взаимодействия стоит не только для аналитики, но и для безопасности.
...
1. Если нагрузка превосходит ожидания, можно задуматься о её ограничении;
2. Если атаки на систему не пережить самим, для защиты можно использовать внешние сервисы;
3. Уже со старта необходимо позаботиться о мониторинге всех составляющих системы;
4. Не стоит забывать и о других командах и их системах, продумайте взаимодействие с ними;
5. При работе с пользовательским контентом надо иметь возможность изучить историю изменений.
...
За трафик он бы не расплатился.

Но он за него и не платит! Netflix очень популярный сервис стриминга видео и при передаче контента от Netflix к пользователю, есть ещё и посредники в виде провайдеров, которым это все передавать.
...
![Image alt](https://github.com/dmatwe/projects/blob/main/System_design/ФТ%20НФТ%20Сервисов/png/netcdn.png)
